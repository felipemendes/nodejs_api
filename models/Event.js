...
const Event = {
    ...
    getEvents: function (category, upcoming, status = 1, uuid, search, page = 1, limit = 10, callback) {
        ...
        const query = "SELECT *, DATE_FORMAT(created_at, '%d/%m/%Y %H:%i:%s') as created_at, DATE_FORMAT(updated_at, '%d/%m/%Y %H:%i:%s') as updated_at, DATE_FORMAT(date, '%d/%m/%Y %H:%i') as date, category.title as category FROM event INNER JOIN category ON event.id_category=category.id" + whereClause + " AND date >= '" + upcomingFilter + "' ORDER BY date LIMIT " + validLimit + " OFFSET " + currentPage + "";
